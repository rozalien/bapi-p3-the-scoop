...
function createComment(url, request) {
  ...
  if ((typeof(request.body) === 'undefined')
      || (typeof(request.body.comment) === 'undefined')
      || requestComment.body === ''
      || requestComment.articleId === ''
      || requestComment.username === ''
      ||!(requestComment.username in database.users)
...
function deleteComment(url, request) {
  ...
  const index = user.commentIds.indexOf(id);
  user.commentIds.splice(index, 1);

  let article = database.articles[savedComment.articleId];
  const artIndex = article.commentIds.indexOf(id);
  article.commentIds.splice(artIndex, 1);
...
function upvote(url, request) {
  ...
  if (user === undefined || username === '' || username === null || !username
    || savedComment === undefined){
      response.status = 400;
      return response;
    }

  let item = database.comments[id];
